...
import {
  ...
  Play,
  AlertTriangle,
  MessageSquare, Star, Frown, MehIcon, SmileIcon, SmilePlus
} from 'lucide-react';
...
import {moderateContent} from "../../services/moderation.service.ts";
...
const RegistrarPresenca = () => {
  ...
  const emojis = [
    { icon: Frown, label: 'Muito Insatisfeito', color: 'text-red-500' },
    { icon: MehIcon, label: 'Insatisfeito', color: 'text-orange-500' },
    { icon: SmileIcon, label: 'Regular', color: 'text-yellow-500' },
    { icon: SmileIcon, label: 'Regular', color: 'text-green-300' },
    { icon: SmilePlus, label: 'Excelente', color: 'text-green-500' }
  ];
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      // Chama o serviço de moderação de conteúdo
      //const moderationResponse = await moderateContent(feedback);


      // Se o conteúdo for apropriado, envia o feedback
      await StudentService.postFeedback({
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          const imageData = context.getImageData(0, 0, canvas.width, canvas.height);
          const code = jsQR(imageData.data, imageData.width, imageData.height);
          console.log(code, 'valor de data')
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-4 text-center text-sm">
                  Seu feedback é anônimo e ajudará a melhorar as próximas aulas
                </p>
                <div className="space-y-6">
                  <div className="space-y-4">
                      ...
                      {emojis.map((item, index) => (
                          <button
                              ...
                              className={`transition-transform hover:scale-110 focus:outline-none ${
                                  selectedEmoji === index ? 'transform scale-110' : ''
                              }`}
                          >
                            <item.icon
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 text-center h-6">
                      {hoveredEmoji !== null ? emojis[hoveredEmoji].label :
                          selectedEmoji !== null ? emojis[selectedEmoji].label :
                  ...
                  <textarea
                      ...
                      placeholder="Como foi a aula de hoje? O que poderia ser melhorado?"
                      className="w-full h-32 px-3 py-2 text-gray-700 dark:text-gray-300 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:border-gray-600"
                  />
